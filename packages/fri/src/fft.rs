...
// fn _simple_ft<T: PrimeField + ScalarOps>(values: &[T], roots_of_unity: &[T]) -> Vec<T> {
...
// fn _fft<T: PrimeField + ScalarOps>(values: &[T], roots_of_unity: &[T]) -> Vec<T> {
//   if values.len() <= 4 {
...
//     let w_m = root_of_unity.pow(&[(order_of_root / (2 * m)) as u64]);
...
//   let new_omega = root_of_unity.pow(&[num_cpus as u64]);
...
//         let omega_j = root_of_unity.pow(&[j as u64]); // omega^j
//         let omega_step = root_of_unity.pow(&[(j as u64) << log_new_n]); // omega^(j * 2**log_new_n)
...
// fn _inv_fft<T: PrimeField + ScalarOps>(values: &[T], roots_rev: &[T]) -> Vec<T> {
...
// pub fn inv_serial_fft<T: PrimeField + FromBytes>(
//   values: &mut [T],
//   root_of_unity: &T,
//   log_order_of_root: u32,
// ) {
//   let order_of_root = 1u32 << log_order_of_root;
//   let m: T = T::from_bytes_be(&order_of_root.to_be_bytes()).unwrap();
//   let inv_len = m.invert().unwrap();
//   let inv_root_of_unity = root_of_unity.invert().unwrap();
//   serial_fft(values, &inv_root_of_unity, log_order_of_root);
//   for i in 0..values.len() {
//     values[i].mul_assign(inv_len);
//   }
// }

// pub fn inv_parallel_fft<T: PrimeField + FromBytes>(
//   values: &mut [T],
//   root_of_unity: &T,
//   worker: &Worker,
//   log_order_of_root: u32,
...
// pub fn mul_polys<T: PrimeField + ScalarOps>(a: &[T], b: &[T], root_of_unity: T) -> Vec<T> {
//   let roots = expand_root_of_unity(root_of_unity);
//   let x1 = _fft(a, &roots);
//   let x2 = _fft(b, &roots);
...
// pub fn div_polys<T: PrimeField + ScalarOps>(a: &[T], b: &[T], root_of_unity: T) -> Vec<T> {
//   let roots = expand_root_of_unity(root_of_unity);
//   let x1 = _fft(a, &roots);
//   let x2 = _fft(b, &roots);
