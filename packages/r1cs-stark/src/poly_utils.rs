...
pub fn eval_poly_at<T: PrimeField + ScalarOps>(polyn: &[T], x: T) -> T {
  let mut y = T::zero();
  let mut power_of_x = T::one();
  for polyn_coeff in polyn {
    let v = power_of_x * polyn_coeff.clone();
    y.add_assign(&v);
...
pub fn mul_by_const<T: PrimeField + ScalarOps>(a: &[T], scalar: T) -> Vec<T> {
  a.iter().map(|x| scalar * x).collect()
}
...
pub fn reduction_poly<T: PrimeField + ScalarOps>(a: &[T], n: usize) -> Vec<T> {
  let mut o = vec![T::zero(); n];
  for i in 0..a.len() {
    o[i % n].add_assign(&a[i]);
  }
...
// recommend to use the DFT in the case of high degree
pub fn mod_polys<T: PrimeField + ScalarOps>(a: &[T], b: &[T]) -> Vec<T> {
...
pub fn zpoly<T: PrimeField + ScalarOps>(xs: &[T]) -> Vec<T> {
...
pub fn lagrange_interp<T: PrimeField + ScalarOps>(
  xs: &[T],
  ys: &[T],
  ...
  for i in 0..xs.len() {
    ...
    for j in 0..ys.len() {
      if nums[i][j] != T::zero() && ys[i] != T::zero() {
        let v = nums[i][j] * yslice;
        b[j].add_assign(&v);
      }
    }
  }

  Polynomial::from_coeffs(b)
}

// Optimized poly evaluation for degree 4
pub fn eval_quartic<T: PrimeField + ScalarOps>(p: [T; 4], x: T) -> T {
  let xsq = x * x;
  let xcb = xsq * x;
